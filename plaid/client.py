import json
from urllib.parse import urljoin #change urlparse with urllib.parse

from http_plaid import http_request
...
class Client(object):
    ...
    def connect(self, account_type, username, password, email, options=None):
        ...
        if response.ok:
            json_data = json.loads(response.content.decode('UTF-8')) #add UTF-8 decoding
